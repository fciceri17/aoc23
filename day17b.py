...
def dijkstra(G, start, goal):
    ...
    while todo:
        ...
        for neighbor, distance in G[location].items():
            ...
            if (neighbor in visited or new_dir_counter>9 or (direction!=previous_direction and dir_counter<3)
                    or neighbor[0]==goal and new_dir_counter<3
            ): continue  # skip these
...
# out_grid = [[out_grid[i][j] for i in range(rows)] for j in range(cols)]
print(cost)
